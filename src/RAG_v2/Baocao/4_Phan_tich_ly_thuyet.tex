...
\begin{itemize}
    \item \textbf{Thu thập và xử lý dữ liệu}: 137 tài liệu từ 3 nguồn (16 PDF quy định, 38 PDF CTDT từ 6 khoa/viện, 83 JSON STSV); 4 loại chunker chuyên biệt tạo ra 2.752 chunks chất lượng cao; Metadata Enrichment tự động trích xuất thời điểm hiệu lực, khóa áp dụng;
    
    \item \textbf{Embedding Layer}: Dual embedding với BGE-M3 (dense + sparse) và E5-Multilingual (1024 chiều), kiến trúc Clean Architecture với Factory Pattern cho phép đổi provider qua \texttt{.env};
    
    \item \textbf{Hybrid Retrieval}: Qdrant Vector Store (dual named vectors) + Elasticsearch BM25, kết hợp qua RRF fusion với cấu hình tối ưu v0.8/k0.2; Multi-Collection Search song song trên 4 collections; Collection Selector chọn collection theo domain confidence;
    
    \item \textbf{Reranking Layer}: Cross-encoder BAAI/bge-reranker-v2-m3 chấm điểm từng cặp (query, document), top-K selection với score threshold;
    
    \item \textbf{Query Router \& Reflection}: Router dual-mode (classifier zero-cost + LLM) phân loại 6 labels; Reflection viết lại query (giải tham chiếu, mở rộng viết tắt, profile extraction tự động);
    
    \item \textbf{Chat Model \& Self Evaluation}: GeminiLLM (Gemini 2.5 Flash) với 3 mode (RAG, chitchat, self\_eval); SelfEvaluator đánh giá relevance, faithfulness, completeness;
    
    \item \textbf{Tool Search và MongoDB Memory}: Tavily web search với rate limiting và exponential backoff làm fallback; MongoDB lưu trữ lịch sử chat, trạng thái phiên, và câu trả lời kèm provenance;
    
    \item \textbf{FastAPI Backend và Pipeline Integration}: RAGPipeline kết nối 8 tầng thành pipeline end-to-end; FastAPI server với SSE streaming, health check, và session management; Pydantic BaseSettings cho centralized configuration.
\end{itemize}
