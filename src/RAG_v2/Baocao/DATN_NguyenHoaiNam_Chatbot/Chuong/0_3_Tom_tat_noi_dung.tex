...
lần lượt đi qua ba mức phân loại độ phức tạp — đối sánh biểu thức chính quy, mô
hình phân loại đa nhãn, và một bước phán đoán bằng LLM khi câu hỏi chạm tới nhiều
miền tri thức — nhằm tách giữa hội thoại xã giao, câu hỏi RAG đơn giản và câu hỏi
phức tạp. Hội thoại xã giao được trả lời trực tiếp mà không kích hoạt truy hồi.
Câu hỏi đơn giản đi theo luồng RAG cổ điển, trong khi các câu hỏi phức tạp, bao
gồm cả dạng so sánh hoặc liên quan nhiều nguồn, được chuyển cho agent dựa trên
LangGraph; khi agent không sẵn sàng hoặc gặp lỗi, hệ thống vẫn có cơ chế quay về
luồng RAG để bảo đảm câu trả lời.

Trước khi truy hồi, lớp xử lý truy vấn kết hợp nhiều thành phần: bộ định tuyến
độ phức tạp, bộ phân loại ý định và miền dữ liệu, bộ chọn collection và bộ phản
chiếu truy vấn. Câu hỏi có thể được viết lại thành truy
...
không đủ hoặc có nhu cầu thông tin mới, cơ chế Tavily web fallback — một thành
phần tùy chọn, mặc định tắt và bật qua cấu hình — cho phép tìm kiếm có kiểm soát
trên các nguồn bên ngoài.
