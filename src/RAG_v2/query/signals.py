...
_WHICH_ORDINAL_RE = re.compile(r"\b(?:ky|ki|thu)\s+may\b")

def _has_how_many_token(query: str, folded: str) -> bool:
    """Return True only for the "mấy" (how many) sense, not "máy" (machine) or "kỳ/thứ mấy" (which ordinal)."""
    folded_hits = len(_FOLDED_MAY_RE.findall(folded))
    machine_hits = len(_MACHINE_ACCENTED_RE.findall(query))
    ordinal_hits = len(_WHICH_ORDINAL_RE.findall(folded))
    return folded_hits > (machine_hits + ordinal_hits)
...
def analyze_query_signals(query: str) -> QuerySignals:
    ...
    # Do not trigger exact policy lookup for "how many" if the query is actually
    # asking for curriculum semester placement ("kỳ mấy").
    exact_policy_lookup = _matches_any(folded, _EXACT_LOOKUP_PATTERNS) or (
        not curriculum_semester_intent and _has_how_many_token(query, folded)
    )
